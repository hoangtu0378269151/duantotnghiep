...
public class ProductController {
	...
	public String list(Model m, @RequestParam("p") Optional<Integer> p, @RequestParam("cid") Optional<String> cid) {
		try {
			...
			if(cid.isPresent()){
				Pageable pa = PageRequest.of(p.orElse(0), 12);
...
//	}
	...
	/*
	 * @RequestMapping("/sale") public String list1(Model m, @RequestParam("p")
	 * Optional<Integer> p) { try { Pageable pa = PageRequest.of(p.orElse(0), 6);
	 * Page<Product> list = productService.findStatus(pa); int a =
	 * list.getTotalPages(); if (list.getNumber() == a) {
	 * System.err.println("Bạn đang ở cuối danh sách"); return
	 * "redirect:/product/list?p=0"; } m.addAttribute("abc", list); } catch
	 * (Exception e) { System.err.println("Bạn đang ở đầu danh sách"); return
	 * "/product/list"; } return "/layout/sale"; }
	 */
