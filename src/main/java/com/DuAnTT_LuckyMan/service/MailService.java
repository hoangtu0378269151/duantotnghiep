...
//		mailSender.send(mimeMessage);
	...
	public void send(String acc,String tongtien) throws MessagingException{
		Locale locale = LocaleContextHolder.getLocale();
		// Prepare the evaluation context
		Context ctx = new Context(locale);
//		ctx.setVariable("message", "Đăng ký thành công");
		ctx.setVariable("message", tongtien);
		// Prepare message using a Spring helper
		MimeMessage mimeMessage = mailSender.createMimeMessage();
		MimeMessageHelper mimeMessageHelper = new MimeMessageHelper(mimeMessage, "UTF-8");
		mimeMessageHelper.setSubject("Hóa đơn thanh toán Luckyman");
		mimeMessageHelper.setTo(acc);
//		mimeMessageHelper.setTo(tongtien);
		// Create the HTML body using Thymeleaf
		String htmlContent = "";
		htmlContent = templateEngine.process("layout/email_en.html", ctx);
		mimeMessageHelper.setText(htmlContent, true);
